...
@cache
def extract_rule2(rule):
    if rule == "42 | 42 8":
        return "(" + extract_rule2("42") + "+)"
    if rule == "42 31 | 42 11 31":
        forth_two = extract_rule2("42")
        thirty_one = extract_rule2("31")
        p11 = []
        for i in range(1, 5):
            times = "{" + str(i) + "}"
    ...
    if "|" not in rule:
        for child in rule.split():
            ps.append(extract_rule2(rules[child]))
        return "".join(ps)
    else:
        for child in rule.split("|"):
            if len(child) == 0:
                continue
            ps.append(extract_rule2(child))
        return "(" + "|".join(f"{item}" for item in ps) + ")"


pattern = extract_rule2(rules["0"])
ans = 0
for text in parts[1].splitlines():
    if re.fullmatch(pattern, text):
        ans += 1

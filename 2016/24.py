...
start = d2p[0]
del d2p[0]
print(min(nx.path_weight(simplified_g, [start] + list(c), weight='weight') for c in permutations(d2p.values())))
print(
    min(nx.path_weight(simplified_g, [start] + list(c) + [start], weight='weight') for c in permutations(d2p.values())))
